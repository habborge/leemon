...
class searchController extends Controller
{
    public function searchProducts(Request $request){
        ...
        $products = Product::where('quantity', '>', 0)->where('name', 'LIKE', "$request->search")->orWhere('brand', 'LIKE', "%$request->search%")->orWhere('name', 'LIKE', "%$request->search%")->orderBy('name', 'ASC')->paginate(24);

        // $products = Product::whereRaw("MATCH(name,brand) AGAINST(?)", array("\"$request->search\"")) 
        // //->orWhereRaw("MATCH(name,brand) AGAINST(? IN BOOLEAN MODE)", array("\"$request->search\""))
